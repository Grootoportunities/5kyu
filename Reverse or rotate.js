...
const revrot = (str, sz) => {
  ...
  return chunks
    .reduce((acc, chunk) => {
      ...
      if (chunkArray.length < sz) return acc;

      const chunkSum = chunkArray.reduce((acc, el) => +acc + +el);

      if (chunkSum % 2 === 0) acc.push(chunkArray.reverse().join(""));
      else {
        const first = chunkArray.shift();

        chunkArray.push(first);
        acc.push(chunkArray.join(""));
      }
